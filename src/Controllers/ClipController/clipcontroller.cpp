...
AVFrame* ClipController::convertQFrameToAVFrame(const QVideoFrame& videoFrame, int width, int height)
{
    ...
    uint8_t* avBuffer = (uint8_t*)av_malloc(numBytes);
    if (!avBuffer) {
        qDebug() << "Failed to allocate buffer for source frame";
        av_frame_free(&srcFrame);
        return nullptr;
    }
    av_image_fill_arrays(srcFrame->data, srcFrame->linesize, avBuffer, AV_PIX_FMT_BGRA, srcFrame->width, srcFrame->height, 1);
...
{
	// Check if there are any buffers to save.
	if (mClipBuffers.isEmpty()) {
		qDebug() << "No clip buffers to save.";
		return;
	}

	// Iterate through each buffer and save them individually.
	for (ClipBufferBase* buffer : mClipBuffers) {
		if (buffer) {
			QString individualFileName = fileName + "_" + QString::number(mClipBuffers.indexOf(buffer)) + ".mp4";
			saveIndividualClip(buffer, individualFileName);
		}
	}
	emit clipSaved(fileName);
}

void ClipController::saveIndividualClip(ClipBufferBase* buffer, const QString& fileName)
{
    // Retrieve buffered QVideoFrame objects.
  //  auto frames = buffer->data();
  //  if (frames.empty()) {
